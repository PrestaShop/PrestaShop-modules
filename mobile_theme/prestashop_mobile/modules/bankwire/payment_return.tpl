...
{if $status == 'ok'}
	<p>{l s='Your order on' mod='bankwire'} <span class="bold">{$shop_name}</span> {l s='is confirmed.' mod='bankwire'}
		<br /><br />
		{l s='Please send us a bank wire with:' mod='bankwire'}
		<br /><br />- {l s='an amount of' mod='bankwire'} <span class="price">{$total_to_pay}</span>
		<br /><br />- {l s='to the order  of' mod='bankwire'} <span class="bold">{if $bankwireOwner}{$bankwireOwner}{else}___________{/if}</span>
		...
		<br /><br />- {l s='Do not forget to include your order number' mod='bankwire'} <span class="bold">{$id_order}</span> {l s='in the subject of your bank wire' mod='bankwire'}
		...
		<br /><br />{l s='For any questions or for further information, please contact ' mod='bankwire'} <a href="{$link->getPageLink('contact-form.php', true)}">{l s='customer support' mod='bankwire'}</a>.
	</p>
{else}
	<p class="warning">
