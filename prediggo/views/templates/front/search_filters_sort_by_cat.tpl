...
<form id="productsSortForm{if isset($aSortingOptions)}{/if}" action="{$request|escape:'htmlall':'UTF-8'}" class="productsSortForm">
    <p class="select">
        <label for="selectProductSortPre{if isset($aSortingOptions)}{/if}">{l s='Sort with Search' mod="prediggo"}</label>
        <select id="selectProductSortPre{if isset($aSortingOptions)}{/if}" class="selectProductSortPre" onChange="location = this.options[this.selectedIndex].value;">
            <option id="test" value="{$orderbydefault|escape:'htmlall':'UTF-8'}:{$orderwaydefault|escape:'htmlall':'UTF-8'}" {if $orderby eq $orderbydefault}selected="selected"{/if}>{l s='--'}</option>
