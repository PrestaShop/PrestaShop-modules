...
class Gsitemap extends Module
{
	...
	'GSITEMAP_FREQUENCY' => 'weekly', 'GSITEMAP_CHECK_IMAGE_FILE' => false, 'GSITEMAP_LAST_EXPORT' => false) as $key => $val)
			if (!Configuration::updateValue($key, $val))
				return false;
		return parent::install() && Db::getInstance()->Execute('CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'gsitemap_sitemap` (`link` varchar(255) DEFAULT NULL, `id_shop` int(11) DEFAULT 0) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8;') && $this->_installOverride();
	}
	...
	{
		return $this->_addLinkToSitemap($link_sitemap, array('type' => 'home', 'page' => 'home', 'link' => $this->context->shop->getBaseURL().(method_exists('Language', 'isMultiLanguageActivated') ? Language::isMultiLanguageActivated() ? $lang['iso_code'].'/' : ''  : ''), 'image' => false), $lang['iso_code'], $index, $i, -1);
	}
	...
	private function _getProductLink(&$link_sitemap, $lang, &$index, &$i, $id_product = 0)
	{
		...
		foreach ($products_id as $product_id)
		{
			...
			if (!$this->_addLinkToSitemap($link_sitemap, array('type' => 'product', 'page' => 'product', 'lastmod' => date_format($product->date_upd, 'c'), 'link' => $url, 'image' => $image_product), $lang['iso_code'], $index, $i, $product_id['id_product']))
				return false;
	...
	private function _getCategoryLink(&$link_sitemap, $lang, &$index, &$i, $id_category = 0)
	{
		...
		foreach ($categories_id as $category_id)
		{
			...
			if (!$this->_addLinkToSitemap($link_sitemap, array('type' => 'category', 'page' => 'category', 'lastmod' => date_format($category->date_upd, 'c'), 'link' => $url, 'image' => $image_category), $lang['iso_code'], $index, $i, (int)$category_id['id_category']))
				return false;
	...
	private function _getManufacturerLink(&$link_sitemap, $lang, &$index, &$i, $id_manufacturer = 0)
	{
		...
		foreach ($manufacturers_id as $manufacturer_id)
		{
			...
			if (!$this->_addLinkToSitemap($link_sitemap, array('type' => 'manufacturer', 'page' => 'manufacturer', 'lastmod' => date_format($manufacturer->date_upd, 'c'), 'link' => $url, 'image' => $manifacturer_image), $lang['iso_code'], $index, $i, $manufacturer_id['id_manufacturer']))
				return false;;
		}
		return true;
	}
	...
	private function _getSupplierLink(&$link_sitemap, $lang, &$index, &$i, $id_supplier = 0)
	{
		...
		foreach ($suppliers_id as $supplier_id)
		{
			...
			if (!$this->_addLinkToSitemap($link_sitemap, array('type' => 'supplier', 'page' => 'supplier', 'lastmod' => date_format($supplier->date_upd, 'c'), 'link' => $url, 'image' => $supplier_image), $lang['iso_code'], $index, $i, $supplier_id['id_supplier']))
				return false;
		}
		return true;
	}
	...
	private function _recursiveSitemapCreator($link_sitemap, $lang, &$index)
	{
		...
		{
			fwrite($writeFd, '<url>'."\r\n");
			$this->_addSitemapNode($writeFd, htmlspecialchars(strip_tags($file['link'])), $this->_getPriorityPage($file['page']), Configuration::get('GSITEMAP_FREQUENCY'), date_format($file['lastmod'], 'c'));
			if ($file['image'])
			{
