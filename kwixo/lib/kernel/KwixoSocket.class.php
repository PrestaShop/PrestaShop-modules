  ...
  function build_header() {
    if ($this->method == 'POST') {
      ...
      if (strlen($this->path . $this->data) > 2048) {
        insertLogKwixo(get_class($this) . " : __construct", "Maximum length in get method reached(" . strlen($this->path . $this->data) . ")");
      }
  ...
  function connect($header) {
    ...
    if ($socket !== false) {
      ...
      //if header sending is impossible : log
        ...
        insertLogKwixo(__METHOD__ . ' : ' . __LINE__, "Envoi des données impossible sur : " . $host);
        $res = false;
      }
      //closes the connexion
      fclose($socket);
    } else { //if connection failed, log
      insertLogKwixo(__METHOD__ . ' : ' . __LINE__, "Connexion socket impossible sur l'hôte $host. Erreur " . $this->errno . " : " . $this->errstr);
      $res = false;
    }
