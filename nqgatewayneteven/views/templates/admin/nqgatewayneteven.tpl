...
<fieldset>
	...
	<p class="neteven-description-title">NetEven, logiciel et services dediés à la gestion des marketplaces européennes</p>
	<p>
		Synchronisez votre site PrestaShop avec Neteven et pilotez tout le cycle de vente sur les marketplaces de manière simple et intégrée.<br />
		Distribuez vos produits en Europe sur les plus grands sites e-commerce depuis une interface unique :  La Redoute - Amazon - eBay – Pixmania – PriceMinister - Cdiscount.com – SFR – RueDuCommerce – Fnac - BrandAlley - OneWorldAvenue - Zalando – Atosho – Play – Hmv - Otto.de - Rakuten.de ...<br />
		Vos produits seront à la disposition de plus de 20 millions d’acheteurs français et 50 millions d’européens !<br/>
		<br />
		<span style="font-size: 1.1em;">Parmi les fonctionnalités disponibles :</span>
	</p>
 
	<ul style="list-style: disc; margin-top: -5px; margin-left: 20px; font-weight: ">
		<li>Inventaire central compatible avec l’ensemble des marketplaces</li>
		<li>Publication par code barre</li>
		<li>Création de fiches produits complètes</li>
		<li>Automatisation des étapes du cycle de vente</li>
		<li>Outils d'animation commerciale</li>
		<li>Fonctionnalité d’ajustement des prix sur le concurrence</li>
		<li>Centralisation des fins de transaction</li>
		<li>Automatisation de la communication client</li>
		<li>Module de Reporting avancé</li>
		<li>Imports / exports personnalisables</li>
		<li>Editions des factures, bordereaux, étiquettes...</li>
	</ul>

	<p class="neteven-description-title">Services d’accompagnement à la distribution</p>
	<p>Appuyez vous sur les experts marketplaces de l’équipe Neteven pour vendre plus et mieux ! Toutes les informations sur les services proposés par NetEven sont disponibles <a href="http://www.neteven.com/offres-logiciel-service.html" target="_blank" style="text-decoration: underline;">ici</a>.</p>
...
<form action="" method="post">
	<fieldset style="margin-top: 15px;" >
		...
		<div class="margin-form">
			<input type="text" name="NETEVEN_LOGIN" value="{$NETEVEN_LOGIN}" />
		</div>
		<label>{l s='Mot de passe NetEven' mod='nqgatewayneteven'}</label>
		<div class="margin-form">
			<input type="password" name="NETEVEN_PASSWORD" value="{$NETEVEN_PASSWORD}" />
		</div>
        ...
        <div class="margin-form">
            <input type="text" name="DEFAULT_BRAND" value="{$DEFAULT_BRAND}" /> <em>({l s='utilisé pour les produits sans marque' mod='nqgatewayneteven'})</em>
        </div>        
        ...
        <div class="margin-form">
            ...
            {foreach from=$format_images item=format_image}
				<option value="{$format_image.name}"{if $format_image.name == $IMAGE_TYPE_NAME} selected="selected"{/if}>{$format_image.name}</option>
			{/foreach}
            </select>
        </div>
    ...
    <fieldset style="margin-top: 15px;" >
        ...
        <div class="margin-form">
            <input type="text" name="SHIPPING_PRICE_LOCAL" value="{$SHIPPING_PRICE_LOCAL}" /> <em>({l s='en' mod='nqgatewayneteven'} {$default_currency->sign})</em>
        </div>
        <label>{l s='Frais de livraison à l\'international' mod='nqgatewayneteven'}</label>
        <div class="margin-form">
            <input type="text" name="SHIPPING_PRICE_INTERNATIONAL" value="{$SHIPPING_PRICE_INTERNATIONAL}" /> <em>({l s='en' mod='nqgatewayneteven'} {$default_currency->sign})</em>
        </div>
        ...
        <div id="shipping_fieldname_container"{if !$SHIPPING_BY_PRODUCT} style="display:none;"{/if}>
            ...
            <div class="margin-form">
                <input type="text" name="SHIPPING_BY_PRODUCT_FIELDNAME" value="{$SHIPPING_BY_PRODUCT_FIELDNAME}" />
            </div>
		</div>
        <br class="clear" />
	...
	<fieldset style="margin-top: 15px;" >
		...
		{if $features}
			{foreach from=$features item=feature}
				<option value="F{$feature.id_feature}">{$feature.name}</option>
			{/foreach}
        {/if}
        {if $attribute_groups}
			{foreach from=$attribute_groups item=attribute_group}
				<option value="A{$attribute_group.id_attribute_group}">{$attribute_group.name}</option>
			{/foreach}
        {/if}
			</select>
		</div>
		...
		<div class="margin-form">
			...
			{foreach from=$neteven_feature_categories item=neteven_features key=neteven_feature_category_name}
				<option value="{$neteven_feature_category_name}" >{$neteven_feature_category_name}</option>
			{/foreach}
            </select>
		</div>
		...
		<div class="margin-form">
			...
			{foreach from=$neteven_feature_categories item=neteven_features key=neteven_feature_category_name}
            	<select rel="{$neteven_feature_category_name}" style="display:none" class="attr_neteven">
                {foreach from=$neteven_features item=neteven_feature}
                	<option value="{$neteven_feature.id_order_gateway_feature}" >{$neteven_feature.name}</option>
                {/foreach}
                </select>
			{/foreach}
		</div>
        ...
        <div id="customizable">
			{if $customizable_fields|@count > 0}
				{foreach from=$customizable_fields item=customizable_field}
                    ...
                    <div class="margin-form">
                        <input type="text" name="customizable_field_name[]" value="{$customizable_field.0}" />
                    </div>
                    <label>{l s='Valeur' mod='nqgatewayneteven'}</label>
                    <div class="margin-form">
                        <input type="text" name="customizable_field_value[]" value="{$customizable_field.1}" />
                    </div>
                    <hr />
				{/foreach}
			{/if}
	...
	<fieldset style="margin-top: 15px;">
        ...
        <div class="margin-form">
            <input type="text" name="NETEVEN_URL" value="{$NETEVEN_URL}" />
        </div>
        <label>{l s='NS NetEven' mod='nqgatewayneteven'}</label>
        <div class="margin-form">
            <input type="text" name="NETEVEN_NS" value="{$NETEVEN_NS}" />
        </div>
        ...
        <div class="margin-form">
            <input type="text" name="MAIL_LIST_ALERT" value="{$MAIL_LIST_ALERT}" /> <em>({l s='séparées par le caractère :' mod='nqgatewayneteven'})</em>
        </div>
