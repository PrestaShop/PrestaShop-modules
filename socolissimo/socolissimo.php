...
class Socolissimo extends CarrierModule {
    ...
    public function install() {
        if (!parent::install() OR !Configuration::updateValue('SOCOLISSIMO_ID', NULL) OR !Configuration::updateValue('SOCOLISSIMO_KEY', NULL) ||
                !Configuration::updateValue('SOCOLISSIMO_URL', 'http://ws.colissimo.fr/pudo-fo-frame/storeCall.do') OR !Configuration::updateValue('SOCOLISSIMO_URL_MOBILE', 'http://ws.colissimo.fr/')
                OR !Configuration::updateValue('SOCOLISSIMO_PREPARATION_TIME', 1) OR !Configuration::updateValue('SOCOLISSIMO_EXP_BEL', true) ||
                ...
                !Configuration::updateValue('SOCOLISSIMO_SUP_BELG', true) OR !Configuration::updateValue('SOCOLISSIMO_SUP', true) OR !Configuration::updateValue('SOCOLISSIMO_USE_FANCYBOX', false) OR !Configuration::updateValue('SOCOLISSIMO_USE_IFRAME', true))
            return false;
    ...
    private function _displayForm() {
		...
		<p  style="width:500px">' . $this->l('To open your SoColissimo account, please contact "La Poste" at this phone number: 3634 (French phone number).') . '</p>
              ...
              <label style="color:#CC0000;text-decoration : underline;">' . $this->l('Update 2.8') . ': </label>
		<div class="margin-form">
		<p  style="width:500px">' . $this->l('If you update this module from a previous version please make sure to uninstall and reinstall the module first, to get the database update') . '</p>
		</div>
		...
		value="' . (float) (Tools::getValue('overcost', number_format(Configuration::get('SOCOLISSIMO_OVERCOST'), 2, '.', ''))) . '" /> Euro HT
		...
		<p>' . $this->l('Url of back office SoColissimo.') . '</p>
               ...
               <label>' . $this->l('Url So Mobile') . ' : </label>
		<div class="margin-form">
		<input type="text" size="45" name="url_so_mobile" value="' . htmlentities(Tools::getValue('url_so_mobile', Configuration::get('SOCOLISSIMO_URL_MOBILE')), ENT_NOQUOTES, 'UTF-8') . '" />
		<p>' . $this->l('Url of back office SoColissimo Mobile. Customers with smartphones or ipad will be redirect there. Warning, this url don\'t allow delivery in belgium') . '</p>
		</div>
		<label>' . $this->l('Display Mode') . ' : </label>
		<div class="margin-form">
			<input type="radio" name="display_type" id="classic_on" value="0" ' . (!Configuration::get('SOCOLISSIMO_USE_FANCYBOX') && !Configuration::get('SOCOLISSIMO_USE_IFRAME') ? 'checked="checked" ' : '') . '/>
			<label class="t" for="classic_on"> Classic </label>
			<input type="radio" name="display_type" id="fancybox_on" value="1" ' . (Configuration::get('SOCOLISSIMO_USE_FANCYBOX') ? 'checked="checked" ' : '') . '/>
			<label class="t" for="fancybox_on"> Fancybox </label>
			<input type="radio" name="display_type" id="iframe_on" value="2" ' . (Configuration::get('SOCOLISSIMO_USE_IFRAME') ? 'checked="checked" ' : '') . '/>
			<label class="t" for="iframe_on"> iFrame </label>
			<p>' . $this->l('Choose your display mode for windows Socolissimo') . '</p>
		</div>
    ...
    public function hookExtraCarrier($params) {
        ...
        if (($country->iso_code == 'FR' || ($country->iso_code == 'BE' && Configuration::get('SOCOLISSIMO_EXP_BEL'))) AND (Configuration::Get('SOCOLISSIMO_ID') != NULL) &&
    ...
    public function isSameAddress($idAddress, $idCart, $idCustomer) {
        ...
        if ($this->upper($psAddress->lastname) != $this->upper($return['prname']) || $psAddress->id_country != $isoCode || $this->upper($psAddress->firstname) != $this->upper($return['prfirstname']) || $this->upper($psAddress->address1) != $this->upper($return['pradress3']) || $this->upper($psAddress->address2) != $this->upper($return['pradress2']) || $this->upper($psAddress->postcode) != $this->upper($return['przipcode']) || $this->upper($psAddress->city) != $this->upper($return['prtown']) || str_replace(array(' ', '.', '-', ',', ';', '+', '/', '\\', '+', '(', ')'), '', $psAddress->phone_mobile) != $return['cephonenumber']) {
