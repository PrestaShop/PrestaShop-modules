...
<form action="{$smarty.server.REQUEST_URI|escape:'htmlall'}" method="post" class="form">
    ...
    <fieldset><legend><img src="{$moduleDir}/logo.gif" alt="" />{l s='Settings' mod='socolissimo'}</legend>
        ...
        <div class="margin-form">
            <input type="text" size="5" name="costseller" onkeyup="this.value = this.value.replace(/,/g, '.');" value="{if isset($costseller)}{$costseller}{else}0{/if}" />
        ...
        <div class="margin-form">
            <input type="text" size="5" name="supcostbelg" onkeyup="this.value = this.value.replace(/,/g, '.');" value="{if isset($supcostbelg)}{$supcostbelg}{else}0{/if}" />
