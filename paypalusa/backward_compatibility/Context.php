...
if ((bool)Configuration::get('PS_MOBILE_DEVICE')) require_once(_PS_MODULE_DIR_.'/mobile_theme/Mobile_Detect.php');
...
class LinkNew extends LinkCore
{
	...
	public function getModuleLink($module, $link, $args=array())
	{
...
class Context
{
	...
	public function __construct()
	{
		...
		$this->cookie = $cookie;
		$this->cart = new CartBackwardModule($cart->id);
		$this->smarty = $smarty;
		$this->link = new LinkNew();
		...
		if ((bool)Configuration::get('PS_MOBILE_DEVICE')) $this->mobile_detect = new Mobile_Detect();
	}
			...
			if ($this->checkMobileContext())
			{
				switch ((int)Configuration::get('PS_MOBILE_DEVICE'))
				{
					case 0: // Only for mobile device
						if ($this->mobile_detect->isMobile() && !$this->mobile_detect->isTablet()) $this->mobile_device = true;
						break;
					case 1: // Only for touchpads
						if ($this->mobile_detect->isTablet() && !$this->mobile_detect->isMobile()) $this->mobile_device = true;
						break;
	...
	{
		if (!self::$instance->shop->getContextType()) return ShopBackwardModule::CONTEXT_ALL;
...
class ControllerBackwardModule
{
	...
	{
		if (_PS_VERSION_ < '1.5') $this->addJS(_PS_JS_DIR_.'jquery/jquery-1.4.4.min.js');
		elseif (_PS_VERSION_ >= '1.5') $this->addJS(_PS_JS_DIR_.'jquery/jquery-1.7.2.min.js');
	}
...
class CustomerBackwardModule extends Customer
{
	...
	public function isLogged($with_guest = false)
	{
		...
		if ($this->logged == 1 && $this->id && Validate::isUnsignedId($this->id) && Customer::checkPassword($this->id, $this->passwd)) return true;
		return false;
	}

}

class CartBackwardModule extends Cart
{
	public $id_shop = 1;
