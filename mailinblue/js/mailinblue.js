...
$(document).ready(
		 ...
		 var token = jQuery("#customtoken").val();
		   ...
		   if(!$('#apikeybox').is(':hidden')){
			...
			$(".sms_order_setting").click(function () {
                    ...
                    type: "POST",
                    async: false,
                    url: base_url + "modules/mailinblue/ajaxOrderSetting.php",
            ...
            $(".Mailin_Sms_Choice").click(function ()
            { 
			   ...
			   if ($('input:radio[name=Mailin_Sms_Choice]:checked').val() == 2)
			{
                jQuery(".multiplechoice").show();
                jQuery(".singlechoice").hide();
            } else {
			...
			if ($('input:radio[name=Mailin_Sms_Choice]:checked').val() == 0)
			{
                jQuery(".multiplechoice").show();
                jQuery(".singlechoice").hide();
            } else {
	...
	$(".script").click(function() {
		...
		$.ajax({
			type : "POST",
			async : false,
			url : base_url + "modules/mailinblue/ajax.php",
	...
	$(".smtptestclick").click(function() {
		...
		$.ajax({
			type : "POST",
			async : false,
			url : base_url+"modules/mailinblue/ajaxsmtpconfig.php",
	...
	$(".managesubscribe").click(function() {
		...
		$.ajax({
			type : "POST",
			async : false,
			url : base_url + "modules/mailinblue/ajaxsubscribeconfig.php",
	...
	$('<div id="ajax-busy"/> loading..')
		.css(
			...
			background : 'white url('+base_url+'modules/mailinblue/img/loader.gif) no-repeat center'
			}).hide().appendTo('body');
	...
	$('.ajax_contacts_href').live('click', function(e){
		...
		$.ajax({
			type : "POST",
			async : false,
			url : base_url + "modules/mailinblue/ajaxcall.php",
	...
	function loadData(page, token) {
		$.ajax({
			...
			async : false,
			url : base_url
				+ "modules/mailinblue/ajaxemailresult.php",
...
function testSmsShipped() {
 ...
 function validate(emailerr,limiter)
 {
   ...
   if( document.notify_sms_mail_form.mailin_notify_email.value == "" || RegexEmail(document.notify_sms_mail_form.mailin_notify_email.value) == false )
   {
     alert(emailerr);
     document.notify_sms_mail_form.mailin_notify_email.focus() ;
     return false;
   }
   if( document.notify_sms_mail_form.mailin_notify_value.value == "" ||
           isNormalInteger( document.notify_sms_mail_form.mailin_notify_value.value)== false )
   {
     alert(limiter);
     document.notify_sms_mail_form.mailin_notify_value.focus() ;
     return false;
   }
