...
class Mailinblue extends Module {
	...
	*/
	...
	{   global $cookie;
		...
		{
		$resultfinal = Db::getInstance()->ExecuteS('SELECT * FROM (SELECT  B.firstname, B.lastname, B.email, B.newsletter AS newsletter
		, C.call_prefix, D.phone_mobile
		FROM '._DB_PREFIX_.'customer AS B
		LEFT JOIN (SELECT * FROM `'._DB_PREFIX_.'address` 
		WHERE `id_address`  IN (SELECT MAX(id_address) FROM `'._DB_PREFIX_.'address` 
		WHERE `date_upd` IN (SELECT MAX(`date_upd`) FROM `'._DB_PREFIX_.'address` WHERE phone_mobile!="" 
		GROUP BY `id_customer`)
		GROUP BY id_customer) AND deleted = 0) D ON B.id_customer=D.id_customer
		LEFT JOIN '._DB_PREFIX_.'country AS C ON C.id_country = D.id_country) AS T WHERE T.email = "'.pSQL($cookie->email).'"');

		$this->newsletter = !empty($resultfinal[0]['newsletter'])?$resultfinal[0]['newsletter'] : '';
		$this->email = !empty($resultfinal[0]['email'])?$resultfinal[0]['email'] : '';
		$phone_mobile = !empty($resultfinal[0]['phone_mobile'])?$resultfinal[0]['phone_mobile'] : '';
		$this->first_name = !empty($resultfinal[0]['firstname'])?$resultfinal[0]['firstname'] : '';
		$this->last_name = !empty($resultfinal[0]['lastname'])?$resultfinal[0]['lastname'] : '';

		$phone_mobile = $this->checkMobileNumber($phone_mobile, (!empty($resultfinal['0']['call_prefix'])?$resultfinal['0']['call_prefix']:''));
		$phone_mobile = (!empty($phone_mobile)) ? $phone_mobile : '';
		...
		Configuration::updateValue('Mailin_Sms_Message_Land_Id', $cookie->id_lang);
	}
	...
	*/
	public function getNewsletterEmails($start, $page)
	{
			return Db::getInstance()->ExecuteS('
							SELECT * FROM ((SELECT  B.email, B.newsletter AS newsletter, 
							C.call_prefix, D.phone_mobile, B.id_customer, D.date_upd
							FROM '._DB_PREFIX_.'customer AS B   
							LEFT JOIN (SELECT * FROM `'._DB_PREFIX_.'address` 
							WHERE `id_address`  IN (SELECT MAX(id_address) FROM `'._DB_PREFIX_.'address` 
							WHERE `date_upd` IN (SELECT MAX(`date_upd`) FROM `'._DB_PREFIX_.'address` WHERE phone_mobile!="" 
							GROUP BY `id_customer`) 
							GROUP BY id_customer) AND deleted = 0) D ON B.id_customer=D.id_customer
							LEFT JOIN '._DB_PREFIX_.'country AS C ON C.id_country = D.id_country) 
							UNION
							(SELECT A.email, A.active AS newsletter,NULL AS call_prefix, 
							NULL AS phone_number, "Nclient" AS id_customer,NULL AS date_upd
							FROM '._DB_PREFIX_.'mailin_newsletter AS A)) As t  LIMIT '.(int)$start.','.(int)$page);
	}
	...
	*/
	public function getTotalEmail()
	{
		return Db::getInstance()->getValue('
							...
							(SELECT A.email, A.active AS newsletter,NULL AS call_prefix, 
							NULL AS phone_number, "Nclient" AS id_customer,NULL AS date_upd
							FROM '._DB_PREFIX_.'mailin_newsletter AS A)) AS tbl');
	}
	...
	*/
	public function updateNewsletterStatus()
	{
		...
		if (isset($this->newsletter) && $this->newsletter != '' && $this->email != '')
		{
			...
			elseif ($this->newsletter == 1)
			{
				...
				if (!empty($data[0]['phone_mobile']) || $data[0]['phone_mobile'] != '')
					$mobile = $this->checkMobileNumber($data[0]['phone_mobile'], $data[0]['call_prefix']);
				else
					$mobile = '';
			...
			Db::getInstance()->Execute('UPDATE `'._DB_PREFIX_.'mailin_newsletter`
	...
	{
		return Db::getInstance()->ExecuteS('
		SELECT * FROM (SELECT  B.firstname, B.lastname, B.email, B.newsletter AS newsletter, 
		C.call_prefix, D.phone_mobile
		FROM '._DB_PREFIX_.'customer AS B   
		LEFT JOIN (SELECT * FROM `'._DB_PREFIX_.'address` 
		WHERE `id_address`  IN (SELECT MAX(id_address) FROM `'._DB_PREFIX_.'address` 
		...
		GROUP BY `id_customer`) 
		GROUP BY id_customer) AND deleted = 0) D ON B.id_customer=D.id_customer
		LEFT JOIN '._DB_PREFIX_.'country AS C ON C.id_country = D.id_country) AS T WHERE T.email = "'.pSQL($email).'"');
	}
	...
	*/
	...
	/**
	* This method is used to fix country code in MailinBlue
	*/
	...
	*/
	public function getBothNewsletteremails()
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('SELECT P.email, P.newsletter as newsletter,
			...
			if (Q.newsletter_date_add is null, 0, "mailin_newsletter_table") as table_type
			from '._DB_PREFIX_.'mailin_newsletter AS Q ');
	}
	...
	public function checkModuleStatus()
	{
		if (_PS_VERSION_ <= '1.4.8.3')
			return Db::getInstance()->getValue('SELECT `active` FROM `'._DB_PREFIX_.'module`
				WHERE `name` = \''.pSQL('mailinblue').'\'');
		elseif (!Module::isEnabled('mailinblue'))
			return false;
		return true;
	}
	...
	*/
	private function autoSubscribeAfterInstallation()
	{
		...
		$register_result = Db::getInstance()->ExecuteS('SELECT * FROM (SELECT  B.newsletter, B.email, B.firstname, B.lastname, 
														C.call_prefix, D.phone_mobile
														FROM '._DB_PREFIX_.'customer AS B   
														LEFT JOIN (SELECT * FROM `'._DB_PREFIX_.'address` 
														WHERE `id_address`  IN (SELECT MAX(id_address) FROM `'._DB_PREFIX_.'address` 
	...
	*/
	public function postProcessConfiguration()
	{
		...
		else
		{
			...
			return $this->l('Your SMTP account is not activated and	therefore you can\'t use MailinBlue SMTP. For more informations
	 ...
	 * It adds the necessary configurations for MailinBlue in PrestaShop which allows
	 * PrestaShop to use sms service the MailinBlue settings.
	 */
	public function saveSmsOrder()
	{
		...
		{
			Configuration::updateValue('Mailin_Sender_Order', Tools::getValue('sender_order'));
			Configuration::updateValue('Mailin_Sender_Order_Message', Tools::getValue('sender_order_message'));
	...
	{
		Configuration::updateValue('Mailin_Notify_Value', Tools::getValue('mailin_notify_value'));
		Configuration::updateValue('Mailin_Notify_Email', Tools::getValue('mailin_notify_email'));
		return $this->redirectPage($this->l('Setting updated'), 'SUCCESS');
	}
	/**
	...
	public function saveSmsShiping()
	{
		...
		{
			Configuration::updateValue('Mailin_Sender_Shipment', $sender_shipment);
			Configuration::updateValue('Mailin_Sender_Shipment_Message', $sender_shipment_message);
			Configuration::updateValue('Mailin_Sms_Message_Land_Id', $cookie->id_lang);
			return $this->redirectPage($this->l('Setting updated'), 'SUCCESS');
		}
	}
	...
	{
		$mailin_sms_choice = Tools::getValue('Mailin_Sms_Choice');

		if ($mailin_sms_choice == 1)
			$this->singleChoiceCampaign();
		else if ($mailin_sms_choice == 0)
	...
	public function multipleChoiceCampaign()
	{
		...
		else
		{
			...
			foreach ($response as $value)
			{
				...
				{
					$number = $this->checkMobileNumber($value['phone_mobile'], $value['call_prefix']);
					$first_name   = (isset($value['firstname'])) ? $value['firstname'] : '';
					$last_name    = (isset($value['lastname'])) ? $value['lastname'] : '';
					$customer_result = Db::getInstance()->ExecuteS('SELECT id_gender,firstname,lastname FROM '._DB_PREFIX_.'customer WHERE `id_customer` = '.
					(int)$value['id_customer']);

					if (strtolower($first_name) === strtolower($customer_result[0]['firstname']) && strtolower($last_name) === strtolower
					($customer_result[0]['lastname']))
					$civili   = (isset($value['id_gender'])) ? $value['id_gender'] : '';
					else
					$civili = '';
					...
					$civiliti = str_replace('{civility}', $civility, $sender_campaign_message);
					$fname = str_replace('{first_name}', $first_name, $civiliti);
	...
	*/
	...
	{
			return Db::getInstance()->ExecuteS('
							SELECT * FROM ((SELECT  B.email, B.id_gender, B.newsletter AS newsletter, 
							C.call_prefix, D.phone_mobile, D.firstname, D.lastname, B.id_customer, D.date_upd
							FROM '._DB_PREFIX_.'customer AS B   
							LEFT JOIN (SELECT * FROM `'._DB_PREFIX_.'address` 
							WHERE `id_address`  IN (SELECT MAX(id_address) FROM `'._DB_PREFIX_.'address` 
							WHERE `date_upd` IN (SELECT MAX(`date_upd`) FROM `'._DB_PREFIX_.'address` WHERE phone_mobile!="" 
							...
							LEFT JOIN '._DB_PREFIX_.'country AS C ON C.id_country = D.id_country) 
							UNION
							(SELECT A.email, A.active AS newsletter,NULL AS call_prefix,NULL AS firstname,NULL AS lastname,NULL AS id_gender, 
							NULL AS phone_number, "Nclient" AS id_customer,NULL AS date_upd
							FROM '._DB_PREFIX_.'mailin_newsletter AS A)) As t WHERE t.phone_mobile!="" ');
	}
	/**
	...
	*/
	public function geSubstMobileNumber()
	{
			return Db::getInstance()->ExecuteS('
							SELECT * FROM ((SELECT  B.email, B.id_gender, B.newsletter AS newsletter, 
							C.call_prefix, D.phone_mobile, D.firstname, D.lastname, B.id_customer, B.newsletter_date_add, D.date_upd
							FROM '._DB_PREFIX_.'customer AS B    
							LEFT JOIN (SELECT * FROM `'._DB_PREFIX_.'address` 
							WHERE `id_address`  IN (SELECT MAX(id_address) FROM `'._DB_PREFIX_.'address` 
							WHERE `date_upd` IN (SELECT MAX(`date_upd`) FROM `'._DB_PREFIX_.'address` WHERE phone_mobile!="" 
							GROUP BY `id_customer`) 
							GROUP BY id_customer) AND deleted = 0) D ON B.id_customer=D.id_customer
							LEFT JOIN '._DB_PREFIX_.'country AS C ON C.id_country = D.id_country WHERE  B.newsletter_date_add >0) 
							UNION
							(SELECT A.email, A.newsletter_date_add AS newsletter_date_add, A.active AS newsletter,NULL AS call_prefix,NULL AS id_gender, 
							NULL AS firstname,NULL AS lastname,NULL AS phone_number, "Nclient" AS id_customer,NULL AS date_upd
							FROM '._DB_PREFIX_.'mailin_newsletter AS A)) As t WHERE t.phone_mobile!="" AND newsletter_date_add>0 ');
	}
	/**
	* Send SMS from MailinBlue.
	*/
	...
	/**
	* show  SMS  credit from MailinBlue.
	*/
	...
	*/
	public function sendMailProcess()
	{
		...
		if ($smtp_result->result->relay_data->status == 'enabled')
		{
			...
			$this->redirectPage($this->l('Your SMTP account is not activated and therefore you can\'t use MailinBlue SMTP. For more informations, Please contact our support to: contact@mailinblue.com'), 'ERROR');
	}
	...
	*/
	public function curlRequest($data)
	{
		...
		$url = 'https://www.mailinblue.com/ws/getamd/'; // WS URL
		else
	...
	*/
	public function createFolderName()
	{
		...
		// AMD services getambassador in MailinBlue
		$this->amdRequest();
	...
	*/
	public function syncronizeBlockCode()
	{
		...
		$this->_second_block_code .= '<style type="text/css">.tableblock tr td{padding:5px; border-bottom:0px;}</style>
			...
			<label> '.$this->l('Activate MailinBlue to manage subscribers').'
			</label>
			</td>
			...
			<a target="_blank" href="'.$this->path.'mailinblue/cron.php?token='.Tools::encrypt(Configuration::get('PS_SHOP_NAME')).'">
			'.$this->l('this link').'</a> ';
	...
	*/
	public function mailSendBySms()
	{
		...
		$smarty->assign('link', '<a target="_blank" href="'.$this->path.'mailinblue/smsnotifycron.php?lang='.
		...
		$smarty->assign('sms_campaign_status', Configuration::get('Mailin_Api_Sms_Campaign_Status'));
		$smarty->assign('Mailin_Notify_Email', Configuration::get('Mailin_Notify_Email'));
		$smarty->assign('sms_shipment_status', Configuration::get('Mailin_Api_Sms_shipment_Status'));
		$smarty->assign('sms_order_status', Configuration::get('Mailin_Api_Sms_Order_Status'));
		$smarty->assign('sms_credit_status', Configuration::get('Mailin_Api_Sms_Credit'));
		$smarty->assign('prs_version', _PS_VERSION_);
		$smarty->assign('Mailin_Notify_Value', Configuration::get('Mailin_Notify_Value'));
		$smarty->assign('Mailin_Sender_Order', Configuration::get('Mailin_Sender_Order'));
		$smarty->assign('Mailin_Sender_Order_Message', Configuration::get('Mailin_Sender_Order_Message'));
		$smarty->assign('Mailin_Sender_Shipment', Configuration::get('Mailin_Sender_Shipment'));
		$smarty->assign('Mailin_Sender_Shipment_Message', Configuration::get('Mailin_Sender_Shipment_Message'));
	...
	*/
	...
	{
		$this->_html .= '<img src="'.$this->path.'mailinblue/img/'.$this->l('mailinblue.png').'"
	...
	private function displayMailin()
	{
		...
		$this->_html .= '
		<fieldset>
		<legend><img src="'.$this->path.$this->name.'/logo.gif" alt="" /> '.$this->l('MailinBlue').'</legend>
...
$this->l('contact@mailinblue.com').'</a><br />'.$this->l('Phone : 0899 25 30 61').'</p>
		...
		'</b><br /><a href="'.$this->l('https://www.mailinblue.com/').'" target="_blank"
		style="color:#268CCD;">'.$this->l('https://www.mailinblue.com/').'</a></p>
		</div>
		<p>'.$this->l('With the MailinBlue plugin, you can find everything you need to easily and efficiently send your emailing campaigns to your prospects and customers. ').'</p>
		<ul class="listt">
	...
	*/
	private function displayForm()
	{
		...
		'<a href="'.$this->l('https://www.mailinblue.com/').'" class="link_action"  target="_blank">&nbsp;'.$this->l('https://www.mailinblue.com/').'</a></label><br />';
		...
		<label>'.$this->l('Activate the MailinBlue module').'</label><div class="margin-form" style="padding-top:5px">
		...
		<input type="text" name="apikey" id="apikeys" value="'.Tools::safeOutput(Configuration::get('Mailin_Api_Key')).'" />&nbsp;
		<span class="toolTip"
	...
	*/
	public function uninstall()
	{
		...
		Configuration::updateValue('Mailin_Api_Sms_Order_Status', 0);
		Configuration::updateValue('Mailin_Api_Sms_shipment_Status', 0);
		Configuration::updateValue('Mailin_Api_Sms_Campaign_Status', 0);
		Configuration::updateValue('Mailin_Sender_Shipment_Message', '');
		Configuration::updateValue('Mailin_Sender_Shipment', '');
		Configuration::updateValue('Mailin_Sender_Order', '');
		Configuration::updateValue('Mailin_Sender_Order_Message', '');
		Configuration::updateValue('Mailin_Notify_Value', '');
		Configuration::updateValue('Mailin_Notify_Email', '');
		Configuration::updateValue('Mailin_Api_Sms_Credit', 0);
		Configuration::updateValue('Mailin_Notify_Cron_Executed', 0);
	...
	public function hookupdateOrderStatus()
	{
		...
		if ($id_order_state == 4)
		{
			...
			$civili = (isset($customer_civility_result['0']['id_gender'])) ? $customer_civility_result['0']['id_gender'] : '';
			else
			$civili = '';
			...
			if (isset($address->phone_mobile) && !empty($address->phone_mobile))
			{
					...
					$arr['from'] = Configuration::get('Mailin_Sender_Shipment');
					$arr['text'] = $msgbody;
	...
	*/
	public function hookLeftColumn($params)
	{
		...
		return $this->display(__FILE__, 'views/templates/front/mailinblue.tpl');
	}
	/**
	...
	*/
	public function hookOrderConfirmation($params)
	{
		...
		if (Configuration::get('Mailin_Api_Sms_Order_Status') && Configuration::get
		('Mailin_Sender_Order') && Configuration::get('Mailin_Sender_Order_Message'))
		{
				$data = array();
				...
				if (isset($address_delivery[0]['phone_mobile']) && !empty($address_delivery[0]['phone_mobile']))
				{
					...
					$civili = (isset($employee->id_gender)) ? $employee->id_gender : '';
					else
					$civili = '';
					...
					$order_date = str_replace('{order_date}', $ord_date."\r\n", $procuct_price);
					$msgbody = str_replace('{order_reference}', $ref_num, $order_date);
					$data['from'] = Configuration::get('Mailin_Sender_Order');
					$data['text'] = $msgbody;
					$data['to'] = $number;
