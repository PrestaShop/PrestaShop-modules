...
class PayuLatam extends PaymentModule {
                ...
                $this->context->smarty->assign(array(
                                ...
                                'credentialInputVar' => array(
                                        'merchant_id' => array(
                                                'name' => 'merchant_id',
                                                'required' => true,
                                                'value' => (Tools::getValue('merchant_id') ? Tools::safeOutput(Tools::getValue('merchant_id')) : Tools::safeOutput(Configuration::get('PAYU_LATAM_MERCHANT_ID'))),
                                                'type' => 'text',
                                        ...
                                        'api_key' => array(
                                                'name' => 'api_key',
                                                'required' => true,
                                                'value' => (Tools::getValue('api_key') ? Tools::safeOutput(Tools::getValue('api_key')) : Tools::safeOutput(Configuration::get('PAYU_LATAM_API_KEY'))),
                                                'type' => 'text',
	...
	public function hookPayment($params) {
		...
		return $this->display(__FILE__, 'payulatam_payment.tpl');
	}
	...
	private function _saveConfiguration() {
		...
		Configuration::updateValue('PAYU_LATAM_MERCHANT_ID', strval(Tools::getValue('merchant_id')));
		Configuration::updateValue('PAYU_LATAM_ACCOUNT_ID', strval(Tools::getValue('account_id')));
		Configuration::updateValue('PAYU_LATAM_API_KEY', strval(Tools::getValue('api_key')));
	...
	private function _createStates() {
		...
		if (!Configuration::get('PAYU_OS_PENDING'))
		{
			...
			if ($order_state->add())
			{
				$source = dirname(__FILE__).'/logo.jpg';
